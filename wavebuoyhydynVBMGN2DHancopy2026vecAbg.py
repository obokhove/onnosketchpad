...
if nwave==1:
    ...
    Forcingterm = ggrav*fd.exp(-0.5*cc*(Zn-hn+Z0-h0)+cc*(Keel+tanalp*(x[1]-Ly)))*fd.conditional(fd.eq(deltacons,0.0),1.0,( fd.sinh(0.5*cc*deltacons) )/(0.5*cc*deltacons))
else:
    Forcingterm = 0.0
...
if nvpcase=="AVF":
    ...
    # Dynamic solver: wavebuoy_nl = fd.NonlinearVariationalSolver(fd.NonlinearVariationalProblem(F_exprnl, result_mixedmp), solver_parameters=solver_parameters5)
    wavebuoy_nl = fd.NonlinearVariationalSolver(fd.NonlinearVariationalProblem(F_exprnl, result_mixedmp))
else:
    wavebuoy_hydrotstaticnl = fd.NonlinearVariationalSolver(fd.NonlinearVariationalProblem(F_hyexprnl, result_mixedmp)) #, solver_parameters=solver_parameters4v)
    # Dynamic solver: wavebuoy_nl = fd.NonlinearVariationalSolver(fd.NonlinearVariationalProblem(F_exprnl, result_mixedmp), solver_parameters=solver_parameters5v)
    #
...
bb = 10*dt**(3/2)
print('SHIT SHIT SHIT bb, dt',bb,dt)

# cc_values = [10, 100.0, 500.0, 1000.0, 2000.0] # was up to 500 1000 (CG2, 14) 2000, 4000 , 4000.0, 5000.0, 7000.0] # , 7000.0, 10000.0]
#
ccc_values = [0.01*bb**(-1), 0.015*bb**(-1), 0.02*bb**(-1), 0.03*bb**(-1), 0.04*bb**(-1),0.05*bb**(-1),0.06*bb**(-1),0.07*bb**(-1),0.1*bb**(-1),0.15*bb**(-1),0.2*bb**(-1),\
             0.25*bb**(-1),0.3*bb**(-1),0.35*bb**(-1),0.4*bb**(-1),0.45*bb**(-1),0.5*bb**(-1),0.55*bb**(-1),0.6*bb**(-1),0.65*bb**(-1),0.7*bb**(-1),0.75*bb**(-1),\
             0.8*bb**(-1),0.85*bb**(-1),0.9*bb**(-1),0.95*bb**(-1),0.975*bb**(-1),bb**(-1),\
             1.1*bb**(-1),1.2*bb**(-1),1.3*bb**(-1),1.4*bb**(-1),1.5*bb**(-1),1.6*bb**(-1),1.7*bb**(-1),1.8*bb**(-1),1.9*bb**(-1), 2*bb**(-1)] # was up to 500 1000 (CG2, 14) 2000, 4000 , 4000.0, 5000.0, 7000.0] # , 7000.0, 10000.0]
# cc_values = [ bb**(-1)] cc_values = [0.01/bb, 0.015/bb, 0.02/bb]
cc_values = [0.01*bb**(-1), 0.015*bb**(-1), 0.02*bb**(-1), 0.03*bb**(-1), 0.04*bb**(-1),0.05*bb**(-1),0.06*bb**(-1),0.07*bb**(-1),0.1*bb**(-1),0.15*bb**(-1),0.2*bb**(-1),\
             0.25*bb**(-1),0.3*bb**(-1),0.35*bb**(-1),0.4*bb**(-1),0.45*bb**(-1),0.5*bb**(-1),0.55*bb**(-1),0.6*bb**(-1),0.65*bb**(-1),0.7*bb**(-1),0.75*bb**(-1),\
             0.8*bb**(-1),0.85*bb**(-1),0.9*bb**(-1),0.95*bb**(-1),0.975*bb**(-1),bb**(-1)]
fstep = 0.1
#for ii in range(11, 41):
#     cc_values.append((ii * fstep) / bb)
