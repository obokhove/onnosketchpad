...
if nvpcase==2: # Energy conserving softened potential lamb eliminated; weak formulations
    ...
    W_expr = ((1/Lx)*vvmpc1*( (Wh-W0)/dt+1-(bb**2/gamm)*fd.ln(abs(Zh/Z0))/(Zh-Z0) ))*fd.dx(degree=vpolyp)
    Fexpr = Z_expr+W_expr
    solver_parameters2 = {
        'mat_type': 'nest',
    ...
    solvelamb_nl = fd.NonlinearVariationalSolver(fd.NonlinearVariationalProblem(Fexpr, result_mixedmpc),solver_parameters=solver_parameters,options_prefix="softball_",
                                                 nullspace=None,pre_jacobian_callback=None,post_jacobian_callback=None,snes_error_if_not_converged=False) # fails
    # solvelamb_nl = fd.NonlinearVariationalSolver(fd.NonlinearVariationalProblem(Fexpr, result_mixedmpc))  # fails, needs nest
# End if
...
while t <= 1.0*(t_end + dt): #
    ...
    if nvpcase==2:
        ...
        H00 = 0.5*W00**2+Z00-(bb**2/gamm)*fd.ln(Z00)
        H11 = 0.5*W11**2+Z11-(bb**2/gamm)*fd.ln(Z11)
...
plt.savefig("figs/waveEcollball2025.png")
