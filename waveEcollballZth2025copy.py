...
if nic == 1: 
    ...
    dtt = np.minimum(0.01,0.0025) # i.e. 
    Nt = 1 # 
    CFL = 0.5
    dt = CFL*dtt # CFL*dtt
...
if nvpcase==2: # Energy conserving softened potential lamb eliminated; weak formulations
    ...
    transition_width = 1e-14
    s = (dZ / transition_width)
    weight = 0.5 * (1 + fd.tanh(-s))  # 1 near dZ=0
    
    RHSt3 = expZ0*(0.5*(Wh+W0)-0.5*(Wh-W0)*dZ+(1/4)*(Wh+W0)*dZ**2-(1/12)*(Wh-W0)*dZ**3) # Taylor fails
    RHSt0 = expZ0*0.5*(Wh+W0) #  works but jumps
    RHStp = expZ0 * (W0 + Wh) / (2 + dZ) # works for small dt only but jumps
    # RHSt3 = expZ0*(0.5*(Wh+W0)-0.5*(Wh-W0)*dZ+(1/4)*(Wh+W0)*dZ**2-(1/12)*(Wh-W0)*dZ**3)
    RHSte = (W0*fd.exp(-Z0)-Wh*fd.exp(-Zh))/(Zh-Z0) + (Wh-W0)*(fd.exp(-Z0)-fd.exp(-Zh))/(Zh-Z0)**2
    RHStheta = fd.conditional(abs(Zh-Z0)<sfac, RHSt0, RHSte)
    # RHStheta = weight * RHStp + (1 - weight) * RHSte # diverges
    Z_expr = ((1/Lx)*vvmpc0*( (Zh-Z0)/dt-RHStheta ))*fd.dx(degree=vpolyp)
    
    RHSw3 = -1 + (bb**2/gamm)*expZ0*(1-0.5*dZ + (1/6)*dZ**2 - (1/24)*dZ**3) #  Taylor fails
    RHSw0 = -1 + (bb**2/gamm)*expZ0 #  works but jumps
    RHSwp = -1 + (bb**2/gamm)*expZ0 * 2 / (2 + dZ) # works for small dt only but jumps
    RHSwe = -1+(bb**2/gamm)*fd.exp(-Z0)*(1-fd.exp(-(Zh-Z0)))/(Zh-Z0)
    # RHSw3 = -1 + (bb**2/gamm)*expZ0*(1-0.5*dZ)
    RHSW = fd.conditional(abs(Zh-Z0)<sfac, RHSw0, RHSwe )
    # RHSW = weight * RHSwp + (1 - weight) * RHSwe # diverges
    ...
    solver_parameters3 = {
        "mat_type": "nest",             # Nested matrix structure for block systems
        "pc_type": "none",
        "snes_type": "newtonls",
        "snes_max_it": 20,
        "snes_rtol": 1e-12,           # Linear solver absolute tolerance
    }
