...
if nvpcase==2: # Energy conserving softened potential lamb eliminated; weak formulations
    ...
    solver_parameters17 = {
        "snes_type": "newtonls",           # Use standard unconstrained Newton
        "snes_linesearch_type": "basic",   # Take full steps across the non-smooth interface
        "snes_divergence_tolerance": 1.e20,   # CRITICAL: Tell PETSc not to abort on iteration 1
        "snes_max_it": 100,                # Give it plenty of room to self-correct the artifact
        "snes_monitor": None,
        "snes_converged_reason": None,
        "snes_atol": 1e-16,
        "snes_rtol": 1e-10,
    ...
    if nJJ==1:
        ...
        solvelamb_nl = fd.NonlinearVariationalSolver(fd.NonlinearVariationalProblem(Fexpr,result_mixedmpc),\
                                                     solver_parameters=solver_parameters16)  # fails, needs nest
# End if
