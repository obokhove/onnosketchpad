...
nwave = 1
if nwave==1:
    # Constants for the Linear Case  delta = Lx/nx/nCG but now: kappa = aa/bb
    delta = Lx/nx
    LL = 2.0 * delta
    u_b = -LL
    dd = bb
    aadbb = aa/bb
    aaddd = aa/dd # a/d (assuming dd is your variable for d)
    # --- CHANGE: New coefficients for C4 potential (7th order) --- coeff_C = aadbb / (8.0*delta**2) coeff_B = aadbb / (16.0*delta**3)
    coeff_B = aadbb / (32.0 * delta**5)
    coeff_C = aadbb / (16.0 * delta**4)
    coeff_A = 0.0 # Curvature is zero for linear
    # --- CHANGE: Updated V_boundary formula for 7th order ---  V_boundary = coeff_B * (u_b**5/5.0 + LL*u_b**4/4.0) + coeff_C * u_b**4/4.0
    V_boundary = coeff_B * (u_b**7/7.0 + LL*u_b**6/6.0) + coeff_C * u_b**6/6.0
    ...
    nC4 = 0
    if nC4==1:
        # 1. Rigorous Force Coefficients (A, B, C, D, E) C4:
        A_f = -aadbb / (16.0 * delta**3)
        B_f = -aadbb / (16.0 * delta**4)
        C_f = -aadbb / (32.0 * delta**5)
        D_f = 0.0
        E_f = (5.0 * aadbb) / (256.0 * delta**7)
        # 2. Potential Coefficients (c0...c4) derived strictly from A...E # These ensure V'(G) == Lambda(G) identically. C4
        c4 = E_f / 9.0
        c3 = (D_f + 8.0 * delta * c4) / 8.0
        c2 = (C_f + 6.0 * delta * c3) / 7.0
        c1 = (B_f + 4.0 * delta * c2) / 6.0
        c0 = (A_f + 2.0 * delta * c1) / 5.0
        # 3. Potential Shift for C0 continuity at G = -delta # V_spline(-delta) = (-2*delta)**5 * c0
        V_at_neg_delta = (-2.0 * delta)**5 * c0
        C_shift = V_at_neg_delta - 0.5 * aadbb * (-delta)**2
    else:
        # 1. Force Coefficients (A_f, B_f, C_f) matching the Quadratic Boundary
        A_f = (aadbb / (8.0 * delta**2)) + (aaddd / (8.0 * delta))
        B_f = (aadbb / (16.0 * delta**3)) - (aaddd / (16.0 * delta**2))
        C_f = -aaddd / (16.0 * delta**3)
        # 2. Potential Coefficients (c0, c1, c2) such that V' = lambda
        c2 = C_f / 6.0
        c1 = (B_f + 4.0 * delta * c2) / 5.0
        c0 = (A_f + 2.0 * delta * c1) / 4.0
        # 3. Potential Shift for C0 continuity at G = -delta
        # V_spline(-delta) = (-2*delta)**4 * c0
        V_at_neg_delta = (16.0 * delta**4) * c0
        # Integral of a(G/b - G^2/d) evaluated at -delta
        V_lin_raw = aa * (delta**2 / (2.0*bb) + delta**3 / (3.0*dd))
        C_shift = V_at_neg_delta - V_lin_raw

    def V_linear_smoothed(G): 
        u = G - delta
        v = G + delta
        # Spline: u^4 * P(v)
        V_spline = u**4 * (c0 + c1*v + c2*v**2)
        # Quadratic: a*(G^2/2b - G^3/3d)
        V_lin = aa * (G**2 / (2.0*bb) - G**3 / (3.0*dd)) + C_shift
        return fd.conditional(fd.lt(G, -delta), V_lin, fd.conditional(fd.lt(G, delta), V_spline, 0.0))
    
    def mean_lin_smoothed(G): 
        u = G - delta
        v = G + delta
        f_spline = u**3 * (A_f + B_f*v + C_f*v**2)
        f_lin = aa * (G/bb - G**2/dd)
        return fd.conditional(fd.lt(G, -delta), f_lin, fd.conditional(fd.lt(G, delta), f_spline, 0.0))
    
    def mean_spline_factored(G_start, G_end):
        # Computes (V_spline(G_end) - V_spline(G_start)) / (G_end - G_start) using factored polynomials for the Quadratic-matched C2 spline.
        u0 = G_start - delta
        u1 = G_end - delta
        # Expand V(u, v) = u^4 * (c0 + c1*v + c2*v**2) into powers of u only:
        # v = u + 2*delta
        q4 = c0 + 2*delta*c1 + (2*delta)**2*c2
        q5 = c1 + 2*(2*delta)*c2
        q6 = c2
        def p_mean(n, a, b):
            # Result of (a^n - b^n) / (a - b)
            return sum((a**(n-1-i) * b**i) for i in range(n))   
        return (q4 * p_mean(4, u1, u0) + q5 * p_mean(5, u1, u0) + q6 * p_mean(6, u1, u0))
    
    def V_linear_smoothedC4(G): # This potential is C3, making the Force C2 and the Hessian C1
        u = G - delta
        v = G + delta
        # --- CHANGE: Powers increased to 7 and 6 ---  V_spline = coeff_B * (0.2*u**5 + 0.25*LL*u**4) + 0.25*coeff_C * u**4
        V_spline = coeff_B * (u**7/7.0 + LL*u**6/6.0) + coeff_C * u**6/6.0
        ...
        #return fd.conditional(fd.lt(G, -delta), V_lin, fd.conditional(fd.lt(G, delta), V_spline, 0.0))
        # Using the u^5 * P(v) form as requested
        V_spline = u**5 * (c0 + c1*v + c2*v**2 + c3*v**3 + c4*v**4)
        V_lin = 0.5 * aadbb * G**2 + C_shift
        return fd.conditional(fd.lt(G, -delta), V_lin, fd.conditional(fd.lt(G, delta), V_spline, 0.0))
    def mean_lin_smoothedC4(G): # mean so (V(G^n+1)-V(G^n))/(G^n+1-G^n))
        u = G - delta
        v = G + delta
        # --- CHANGE: Powers increased to 6 and 5 --- mean_spline = coeff_B * (u**4 + LL*u**3) + coeff_C * u**3
        # mean_spline = coeff_B * (u**6 + LL*u**5) + coeff_C * u**5
        # mean_lin = aadbb*G
        # return fd.conditional(fd.lt(G, -delta), mean_lin, fd.conditional(fd.lt(G, delta), mean_spline, 0.0))
        # Using the u^4 * P(v) form derived for the force lambda
        f_spline = u**4 * (A_f + B_f*v + C_f*v**2 + D_f*v**3 + E_f*v**4)
        return fd.conditional(fd.lt(G, -delta), aadbb * G, fd.conditional(fd.lt(G, delta), f_spline, 0.0))

    def mean_spline_factoredC4(G_start, G_end):
        """
        ...
        u0 = G_start - delta
        u1 = G_end - delta
        # The 'Mean' of u^4 over [u0, u1] is (u1^5 - u0^5)/(5*(u1-u0)) # Which factors to: (u1^4 + u1^3*u0 + u1^2*u0^2 + u1*u0^3 + u0^4) / 5 sum4 = (u1**4 + u1**3*u0 + u1**2*u0**2 + u1*u0**3 + u0**4)
        # The 'Mean' of u^3 over [u0, u1] is (u1^4 - u0^4)/(4*(u1-u0)) # Which factors to: (u1^3 + u1^2*u0 + u1*u0^2 + u0**3) / 4 sum3 = (u1**3 + u1**2*u0 + u1*u0**2 + u0**3)
        # coeff_B corresponds to the u^5 and u^4 terms in V # coeff_C corresponds to the u^4 term in V # LL = 2 * delta term_B = coeff_B * (0.2 * sum4 + 0.25 * LL * sum3) nterm_C = 0.25 * coeff_C * sum3
        # --- CHANGE: Added higher order factored sums ---
        #sum6 = (u1**6 + u1**5*u0 + u1**4*u0**2 + u1**3*u0**3 + u1**2*u0**4 + u1*u0**5 + u0**6)
        #sum5 = (u1**5 + u1**4*u0 + u1**3*u0**2 + u1**2*u0**3 + u1*u0**4 + u0**5) # --- CHANGE: Adjusted terms for 1/7 and 1/6 weights ---
        #term_B = coeff_B * (sum6/7.0 + LL * sum5/6.0) #term_C = coeff_C * sum5/6.0 #return term_B + term_C
        
        # Expand V(u, v) = u^5 * P(v) into powers of u only: V = sum q_i * u^i
        # since v = u + 2*delta
        q5 = c0 + 2*delta*c1 + (2*delta)**2*c2 + (2*delta)**3*c3 + (2*delta)**4*c4
        q6 = c1 + 2*(2*delta)*c2 + 3*(2*delta)**2*c3 + 4*(2*delta)**3*c4
        q7 = c2 + 3*(2*delta)*c3 + 6*(2*delta)**2*c4
        q8 = c3 + 4*(2*delta)*c4
        q9 = c4
        ...
        return (q5 * p_mean(5, u1, u0) +  q6 * p_mean(6, u1, u0) + q7 * p_mean(7, u1, u0) +  q8 * p_mean(8, u1, u0) +  q9 * p_mean(9, u1, u0))
        
    def get_AVF_Force_Symmetric(G0, G1): # get_AVF_Force_Symmetric and Forcingterm logic remains exactly the same
...
if nsa == 1:    
    ...
    h_expr   = (  vvmp1*(phin-phi0)/dt +\
                  ...
                  vvmp1*(grav*(0.5*(hn+h0)-H0)+ Forcingterm + grav*gravwm )  )*fd.dx(degree=vpolyp)
    ...
    h_expr   = (  vvmp1*(phin-phi0)/dt+\
                  vvmp1*( Forcingterm + grav*gravwm ) )*fd.dx(degree=vpolyp) + delHamdelh
...
if nwave==1:
    ...
    Z_expr   = ( vvmp3*( Area*meff*(Wn-W0)/dt+Area*meff*grav-Forcingterm+Area*gam*GZZ12*0.5*(In+I0)/rho0 ))*fd.dx(degree=vpolyp)
...
lam1.interpolate( nwave*( mean_lin_smoothed(Ch) ) )
...
while t <= 1.0*(t_end + dt):
    ...
    if (t in t_plot): # Plotting # print('Plotting starts')
        ...
        E1 = fd.assemble( ( 0.5*hn*fd.inner(fd.grad(phin)+ah01*hn*qn*fd.grad(hn)+aq01*(1/3)*hn**2*fd.grad(qn),fd.grad(phin)+ah01*hn*qn*fd.grad(hn)+aq01*(1/3)*hn**2*fd.grad(qn))+0.5*grav*fd.inner(hn-H0,hn-H0) \
                            ...
                            + V_linear_smoothed( (Zn-hn-Keel-tanalp*(x[1]-Ly)) ) \
                            + nwave*Area*meff*(0.5*Wn**2+grav*Zn) + Area*0.5*Li*In**2/rho0 )*fd.dx(degree=vpolyp) )
