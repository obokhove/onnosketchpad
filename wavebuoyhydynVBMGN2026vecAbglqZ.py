...
h_hyexpr   = ( vvmp1*( grav*(hn-H0)-\
                       (1.0)*fd.conditional( fd.lt(Ch,-delta), aadbb*Ch, fd.conditional( fd.lt(Ch, delta), coeff_B*((Ch-delta)**4+LL*(Ch-delta)**3)+coeff_C*(Ch-delta)**3, 0.0) )\
                      ))*fd.dx(degree=vpolyp)
q_hyexpr   = ( vvmp2*( qn ))*fd.dx(degree=vpolyp)   # nil/rest solution
Z_hyexpr   = ( vvmp3*( Area*Mm*grav+\
                       rho0*(1.0)*fd.conditional( fd.lt(Ch,-delta), aadbb*Ch, fd.conditional( fd.lt(Ch, delta), coeff_B*((Ch-delta)**4+LL*(Ch-delta)**3)+coeff_C*(Ch-delta)**3, 0.0) )\
                      ))*fd.dx(degree=vpolyp)
