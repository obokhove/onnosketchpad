...
# =================================================
# Onno Bokhove 05-02-2025 04-04-2026 made update to A*exp(-gamma*(hb-h)/b)
#
# .. rst-class:: emphasis https://www.firedrakeproject.org/checkpointing.html#checkpointing-with-checkpointfile https://www.firedrakeproject.org/optimising.html
#
#     Contributed by `Onno Bokhove <mailto:O.Bokhove@leeds.ac.uk>`__. https://github.com/firedrakeproject/firedrake/blob/main/firedrake/solving_utils.py
#
# Time-step choices: AVF energy-conserving approach
# 15-02 new mesh 2000 CG2: 91.13085844914119 16-02 CG3 new mesh 2000: 479.34392260313035
# Initial conditions/tests: Elapsed time (min): 125.43053943316141 CG2 474.2686847368876 CG3 CG1 crica 25min
#
...
def asinh_ufl(x):
    return fd.ln(x + fd.sqrt(1.0 + x*x))
...
if nbah==1:
    ...
    from meshingbahtest import get_mesh
elif nbah==4:
    from meshingtest import get_mesh
else:
    from meshing import get_mesh
...
nvpcase = "AVFvector" # AVF= VP time discretisation energy-conserving approach
...
# CG1 07-02-2026 CG1 standard solve AVFvector:  20.807460395495095 ;  CG! AVFvector default again: 20.71506766875585
# AVFvector parametersolve5: 20.719536817073823   48.87570768594742    ; AVF with solve5:   48.970563431580864 63.89474473396937 AVF sole default: 22.807149962584177
# AVFvector default CG3 CFL 0.5*0.7*0.125*fac: 633.337923681736
# AVF default CG3 CFL 0.5*0.7*0.125*fac: 647.2542177518209
Lxx = Lx
Lyy = Ly
nxx = nx
nyy = ny
...
if nserial==1:
    ...
    with fd.CheckpointFile("custom_meshtest.h5", "w") as chk:
        chk.save_mesh(mesh)
elif nserial==2:
    mesh2d = get_mesh(Lxx, Lyy, nxx+1, nyy, Lr)[0]
    mesh = mesh2d
...
V_vec = fd.VectorFunctionSpace(mesh, 'CG', nCG, vfamily='CG',dim=3) # interior potential varphi; can mix degrees in hor and vert
# mesh_global = fd.UnitIntervalMesh(1) 
# V_R = fd.FunctionSpace(mesh_global, 'DG', 0, vfamily='DG') # interior potential varphi; can mix degrees in hor and vert
# OLD
...
if nvpcase=="AVFvector":
    # Variables at midpoint for modified midpoint waves mixed_vmp = fd.MixedFunctionSpace([V_vec, V_R, V_R, V_R])
    #OLD
    mixed_Vmp = V_vec * V_R * V_R * V_R
    result_mixedmp = fd.Function(mixed_Vmp)
    vvmp = fd.TestFunction(mixed_Vmp)
    vvmpvec, vvmp3, vvmp4, vvmp5 = fd.split(vvmp) # These represent "blocks".
    Un, Zn, Wn, In= fd.split(result_mixedmp) #
    phin = Un[0]
    hn = Un[1]
    qn = Un[2]
    vvmp0 = vvmpvec[0]
...
set0 = 1.0
setq = 1.0
print('SHIT SHIT SHIT bbb, dt',bbb,dt)

Ch = Zn-Keel-tanalp*(x[1]-Ly)-hn
C0 = Z0-Keel-tanalp*(x[1]-Ly)-h0
deltacons = Ch-C0
nwave = 1
if nwave==1:
    # Constants for the Linear Case  delta = Lx/nx/nCG but now:
    delta = Lx/nx
    LL = 2.0 * delta
    u_b = -LL
    aadbb = aa/bb
    coeff_C = aadbb / (8.0*delta**2)
    coeff_B = aadbb / (16.0*delta**3)
    coeff_A = 0.0 # Curvature is zero for linear
    V_boundary = coeff_B * (u_b**5/5.0 + LL*u_b**4/4.0) + coeff_C * u_b**4/4.0
    V_lin_raw = 0.5 * aadbb * delta**2 # Raw linear potential at G = -delta: 0.5 * (a/b) * (-delta)**2
    C_shift = V_boundary-V_lin_raw # The shift ensures V_linear(-delta) == V_spline(-delta)
    def V_linear_smoothed(G): # This potential is C3, making the Force C2 and the Hessian C1
        u = G - delta
        V_spline = coeff_B * (0.2*u**5 + 0.25*LL*u**4) + 0.25*coeff_C * u**4
        # Linear potential for G < -delta # Shifted by C_shift to match V_spline(-delta) v_at_minus_delta = (aa/bb) * (delta**2 / 2.0) # approx
        V_lin = aadbb * (0.5 * G**2) + C_shift
        return fd.conditional(fd.lt(G, -delta), V_lin, fd.conditional(fd.lt(G, delta), V_spline, 0.0))
    def mean_lin_smoothed(G): # mean so (V(G^n+1)-V(G^n))/(G^n+1-G^n))
        u = G - delta
        mean_spline = coeff_B * (u**4 + LL*u**3) + coeff_C * u**3
        mean_lin = aadbb*G
        return fd.conditional(fd.lt(G, -delta), mean_lin, fd.conditional(fd.lt(G, delta), mean_spline, 0.0))

    def mean_spline_factored(G_start, G_end):
        """
        Computes (V_spline(G_end) - V_spline(G_start)) / (G_end - G_start)
        using pure polynomial expansion to avoid division by zero.
        """
        # Shift to the spline coordinate u = G - delta
        u0 = G_start - delta
        u1 = G_end - delta
        # The 'Mean' of u^4 over [u0, u1] is (u1^5 - u0^5)/(5*(u1-u0)) # Which factors to: (u1^4 + u1^3*u0 + u1^2*u0^2 + u1*u0^3 + u0^4) / 5
        sum4 = (u1**4 + u1**3*u0 + u1**2*u0**2 + u1*u0**3 + u0**4)
        # The 'Mean' of u^3 over [u0, u1] is (u1^4 - u0^4)/(4*(u1-u0)) # Which factors to: (u1^3 + u1^2*u0 + u1*u0^2 + u0**3) / 4
        sum3 = (u1**3 + u1**2*u0 + u1*u0**2 + u0**3)
        # coeff_B corresponds to the u^5 and u^4 terms in V # coeff_C corresponds to the u^4 term in V # LL = 2 * delta
        term_B = coeff_B * (0.2 * sum4 + 0.25 * LL * sum3)
        term_C = 0.25 * coeff_C * sum3
        return term_B + term_C
    
    def get_AVF_Force_Symmetric(G0, G1):
        # 1. Sort the points to ensure symmetry and simplify crossing logic
        g_min = fd.min_value(G0, G1)
        g_max = fd.max_value(G0, G1)
        # Total displacement (The "B-Proof" denominator) # We use a small epsilon only for the final division to avoid 0/0
        delta_G = g_max - g_min
        safe_dg = fd.max_value(delta_G, 1e-12)
        # 2. Define the Region boundaries
        at = -delta
        bs = delta
        # 3. Calculate segment lengths (L_i)
        # These represent how much of the path [g_min, g_max] lies in each region. # Region 0: (-inf, a)
        L0 = fd.max_value(0.0, fd.min_value(at, g_max) - g_min)
        # Region 1: [at, bs] # Length is: (right boundary of path in R1) - (left boundary of path in R1)
        r1_right = fd.min_value(bs, g_max)
        r1_left = fd.max_value(at, g_min)
        L1 = fd.max_value(0.0, r1_right - r1_left)
        # Region 2: (bs, +inf)
        L2 = fd.max_value(0.0, g_max - fd.max_value(bs, g_min))
        # 4. Define Regional "Means" (Endpoints for each segment)
        # Segment 0 boundaries
        s0 = g_min
        e0 = fd.min_value(at, g_max)
        m0 = aadbb * 0.5 * (s0 + e0)
        # Segment 1 boundaries (The Spline)
        s1 = fd.max_value(at, g_min)
        e1 = fd.min_value(bs, g_max)
        # Use the factored polynomial we derived to keep the Jacobian clean
        m1 = mean_spline_factored(s1, e1)
        # Segment 2 boundaries (Air)
        m2 = 0.0 
        # 5. The Weighted Sum (Total Integral / Total Length)
        # This is mathematically equivalent to (V(G1) - V(G0)) / (G1 - G0) # But because it is expressed as a sum of segments, it is much  # more stable for the Newton Solver.
        AVF_sum = (L0 * m0 + L1 * m1 + L2 * m2) / safe_dg
        # Final check: If displacement is near zero, use the local derivative 
        # to provide a perfect Jacobian for the linear solver.
        return fd.conditional(fd.lt(delta_G, 1e-10), mean_lin_smoothed(G1),AVF_sum)
    
    def V_lamb5(G): # Potential for lambda5(G)
        Gdb = G/bb 
        return fd.conditional(fd.ge(G,0.0), 0.0, aa*( G-0.5*bb*(Gdb*fd.sqrt(1.0+Gdb**2)+asinh_ufl(Gdb)) ) )
        return 

    def manual_jacobian_term(Gnp1, Gn):
        Ghat_np1 = fd.conditional(fd.ge(Gnp1, 0.0), 0.0, Gnp1)
        Ghat_n   = fd.conditional(fd.ge(Gn, 0.0), 0.0, Gn)
        V_np1 = V_lamb6(Ghat_np1)
        V_n   = V_lamb6(Ghat_n)
        denom = Gnp1 - Gn
        # The derivative of V(Ghat) w.r.t Gnp1 # If Gnp1 < 0: d/dGnp1 V(Gnp1) = V'(Gnp1) = lamb66(Gnp1)
        # If Gnp1 >= 0: d/dGnp1 V(0) = 0 (because V'(0)=0) # We define this explicitly to avoid UFL's conditional derivative bug.
        dV_dG = fd.conditional(fd.lt(Gnp1, 0.0), lamb66(Gnp1), 0.0)
        # The full Jacobian expression # J = - [ dV_dG * denom - (V_np1 - V_n) ] / denom^2
        # Handle denom=0 case separately if needed, but Newton usually handles it if the expression is smooth. # Note: If denom is very small, this formula might be unstable.
        # But the key is that dV_dG is now a smooth conditional (0 or lamb66) without the "delta" issue.
        numerator = dV_dG * denom - (V_np1 - V_n)
        denominator = denom * denom
        # Avoid division by zero in the symbolic graph if Gnp1 == Gn # Use a small epsilon ONLY for the denominator in the Jacobian if necessary, # but the numerator should be 0 when denom is 0.
        # Actually, if Gnp1 == Gn, numerator = 0*0 - 0 = 0. # So the limit is well-defined.
        return - numerator / denominator
    
    def V_lamb6(G): # Potential for lambda5(G)
        Gdb = G/bb 
        return fd.conditional(fd.ge(G,0.0), 0.0, aa*( G+G**3/(6*bb**2)-0.5*bb*(Gdb*fd.sqrt(1.0+Gdb**2)+asinh_ufl(Gdb)) ) )
        # return aa*( G-G**3/(6*bb**2)-0.5*bb*(Gdb*fd.sqrt(1.0+Gdb**2)+asinh_ufl(Gdb)) )

    def lamb66(G):
        Gdb = G/bb
        #
        return fd.conditional(fd.ge(G,0.0), 0.0, -aa*( -0.5*G**2/bb**2+fd.sqrt(1.0+Gdb**2)-1 ) )
        # return -aa*( 0.5*G**2/bb**2+fd.sqrt(1.0+Gdb**2)-1 )

    def lamb6(G): # Used for static case and plotting, so clipped
        Gdb = G/bb
        macht = 4
        # return fd.conditional(fd.ge(G,0.0), 0.0, -aa*( -0.5*G**2/bb**2+fd.sqrt(1.0+Gdb**2)-1 ) )
        return fd.conditional(fd.ge(G,0.0), 0.0, -aa*( (1.0+Gdb**macht)**(1/macht) -1 ) )

    def lambda5_avf_term(Gnp1, Gn): # Clipped variables:   \hat Z = min(0,Z) and  x = \hat Z/b: Gnp1 = fd.conditional(fd.lt(abs(Gnp1-Gn),10**(-14)), Gn+2*10**(-14), Gnp1)
        Ghat_np1 = fd.conditional(fd.ge(Gnp1,0.0), 0.0, Gnp1)
        Ghat_n   = fd.conditional(fd.ge(Gn,0.0), 0.0, Gn)
        geom = fd.conditional( fd.eq(Gnp1,Gn), lamb66(Ghat_np1), (V_lamb6(Ghat_np1)-V_lamb6(Ghat_n))/(Gnp1-Gn) )
        # geom = (V_lamb6(Ghat_np1)-V_lamb6(Ghat_n))/(Gnp1-Gn)
        return -geom

    # Jman = manual_jacobian_term(Ch, C0) * test_function * trial_function
    Forcingterm = lambda5_avf_term(Ch, C0)
else:
    # x_np1 = Ghat_np1/bb
    # x_n   = Ghat_n/bb
    # --------------------------------------------------------------
    # Hyperbolic averages: sqrt_np1 = fd.sqrt(1.0 + x_np1**2) sqrt_n   = fd.sqrt(1.0 + x_n**2) S = fd.sqrt(1.0 + x_np1**2) + fd.sqrt(1.0 + x_n**2) # sqrt_np1 + sqrt_n
    # --------------------------------------------------------------
    # eta = 1/2 (asinh_ufl(x_{n+1}) - asinh(x_n)): eta = 0.5 * (asinh_ufl(x_np1) - asinh_ufl(x_n)) eta = asinh_ufl(x_np1*fd.sqrt(1+x_n**2)-x_n*fd.sqrt(1+x_np1**2))
    # --------------------------------------------------------------
    # Smooth AVF geometry term: fails? geom = ( 1.0 - 0.25*(S+(x_np1+x_n)**2/S) - (eta/tanh(eta))/S )
    # geom0 = ( 1.0 - 0.25*(S+(x_np1+x_n)**2/S) - 0.5*eta/(x_np1-x_n) )
    # geom1 = (V_lamb5(Gnp1)-V_lamb5(Gn))/(Gnp1-Gn) geom2 = (V_lamb6(Gnp1)-V_lamb6(Gn))/(Gnp1-Gn)
    # geom = fd.conditional( fd.eq(Gnp1,Gn), 1.0 - 0.25*(S+(x_np1+x_n)**2/S) - 0.5/fd.sqrt(1.0+x_np1**2), ( 1.0 - 0.25*(S+(x_np1+x_n)**2/S) - 0.5*eta/(x_np1-x_n) ) )
    # geom = fd.conditional( fd.eq(Gnp1,Gn), 1.0 - 0.25*(S+(x_np1+x_n)**2/S) - 0.5/fd.sqrt(1.0+x_np1**2), (V_lamb5(Gnp1)-V_lamb5(Gn))/(Gnp1-Gn) )
    # geom = fd.conditional( fd.eq(Gnp1,Gn), 1.0 -(Gnp1**2+Gnp1*Gn+Gn**3)/(6*bb**2) - 0.25*(S+(x_np1+x_n)**2/S) - 0.5/fd.sqrt(1.0+x_np1**2), (V_lamb6(Gnp1)-V_lamb6(Gn))/(Gnp1-Gn) )
    # --------------------------------------------------------------
    # Exact clipping factor: is the discrete derivative of min(0,Z):
    # --------------------------------------------------------------
    # clip = ( 0.5*(1.0 - (abs(Gnp1)-abs(Gn))/(Gnp1-Gn) ) ) clip = 1.0
    # denom = Gnp1-Gn clip_equal = fd.conditional(fd.lt(Gnp1, 0.0), 1.0, 0.0)
    # clip = fd.conditional(fd.eq(denom,0.0), clip_equal, (Ghat_np1 - Ghat_n) / denom) clip = fd.conditional(abs(denom) < 1e-12, clip_equal, (Ghat_np1 - Ghat_n) / denom)    
    # clip = fd.conditional( fd.eq(Gnp1,Gn), 1.0, (x_np1-x_n)/(Gnp1/bb-Gn/bb) )
    # --------------------------------------------------------------
    # Final AVF contribution: no idea why minus  
    # AVF Step: Forcingterm =  (V_linear_smoothed(Ch) - V_linear_smoothed(C0)) / (deltacons + 1e-15))
    # Forcingterm = -fd.conditional( fd.eq(deltacons,0.0), mean_lin_smoothed(Ch), (V_linear_smoothed(Ch) - V_linear_smoothed(C0))/deltacons )
    # Forcingterm = -fd.conditional( fd.eq(deltacons,0.0), mean_lin_smoothed(Ch), (V_linear_smoothed(Ch) - V_linear_smoothed(C0))/deltacons )
    #  Works but lambda less but still grainy after t=3.5s Forcingterm = -fd.conditional( fd.eq(deltacons,0.0), mean_lin_smoothed(Ch), mean_lin_smoothed(0.5*(Ch+C0)) )
    # Forcingterm = -fd.conditional( fd.eq(deltacons,0.0), mean_lin_smoothed(Ch), get_AVF_Force_Symmetric(C0, Ch) )
    Forcingterm = 0.0
    
# VP or weak forms hydrostatic rest case: fd.conditional( fd.lt(Ch,-delta), aadbb*Ch, fd.conditional( fd.lt(Ch, delta), coeff_B*((Ch-delta)**4+LL*(Ch-delta)**3)+coeff_C*(Ch-delta)**3, 0.0) )\
phi_hyexpr = ( vvmp0*( phin ))*fd.dx(degree=vpolyp) # nil/rest solution # fd.conditional( fd.ge(Ch,0.0), 0.0, -aa*(0.5*Ch**2/(6*bb**2)+fd.sqrt(1.0+(Ch/bb)**2)-1.0) )\
h_hyexpr   = ( vvmp1*( grav*(hn-H0)-\
                            lamb6(Ch)\
                      ))*fd.dx(degree=vpolyp)
q_hyexpr   = ( vvmp2*( qn ))*fd.dx(degree=vpolyp)   # nil/rest solution # fd.conditional( fd.ge(Ch,0.0), 0.0, -aa*(0.5*Ch**2/(6*bb**2)+fd.sqrt(1.0+(Ch/bb)**2)-1.0) )\
Z_hyexpr   = ( vvmp3*( Area*Mm*grav+\
                       rho0*lamb6(Ch)\
                      ))*fd.dx(degree=vpolyp)
...
# VPshit or weak forms hydrostatic rest case ccinv=1/cc = bb
phi_hyexprsh = ( vvmp0*( phin ))*fd.dx(degree=vpolyp) # nil/rest solution
h_hyexprsh   = ( vvmp1*( grav*(hn-H0)+ggrav*ccinv*fd.exp(-cc*(Zn-hn-Keel-tanalp*(x[1]-Ly))) ))*fd.dx(degree=vpolyp)
q_hyexprsh   = ( vvmp2*( qn ))*fd.dx(degree=vpolyp)   # nil/rest solution
# Zshit = ( Area*Mm*grav-rho0*ggrav*ccinv*fd.exp(-cc*(Zn-hn-Keel-tanalp*(x[1]-Ly))) )
Z_hyexprsh   = ( vvmp3*( Area*Mm*grav-rho0*ggrav*ccinv*fd.exp(-cc*(Zn-hn-Keel-tanalp*(x[1]-Ly))) ))*fd.dx(degree=vpolyp)
W_hyexprsh   = ( vvmp4*( Wn ))*fd.dx(degree=vpolyp)   # nil/rest solution
I_hyexprsh   = ( vvmp5*( In ))*fd.dx(degree=vpolyp)   # nil/rest solution
F_hyexprnlsh = phi_hyexprsh+h_hyexprsh+q_hyexprsh+Z_hyexprsh+W_hyexprsh+I_hyexprsh
...
if nsa == 1:    
    ...
    h_expr   = (  vvmp1*(phin-phi0)/dt +\
                  ...
                  vvmp1*(grav*(0.5*(hn+h0)-H0) - lamb6(0.5*(Ch+C0)) + grav*gravwm )  )*fd.dx(degree=vpolyp)
    h_exprforce   = ( vvmp1*( Forcingterm )  )*fd.dx(degree=vpolyp)
    ...
    h_exprm   = ( vvmp1*(phin-phi0)/dt+ vvmp1*( -lamb6(0.5*(Ch+C0)) + grav*gravwm ) )*fd.dx(degree=vpolyp) + delHamdelh
    h_expr   = ( vvmp1*(phin-phi0)/dt+ vvmp1*( 0.0 + grav*gravwm ) )*fd.dx(degree=vpolyp) + delHamdelh
    h_exprforce   = ( vvmp1*( Forcingterm ) )*fd.dx(degree=vpolyp) + delHamdelh
...
if nwave==1:
    ...
    Z_exprm   = ( vvmp3*( Area*meff*(Wn-W0)/dt+Area*meff*grav +lamb6(0.5*(Ch+C0)) +Area*gam*GZZ12*0.5*(In+I0)/rho0 ))*fd.dx(degree=vpolyp)
    Z_expr   = ( vvmp3*( Area*meff*(Wn-W0)/dt+Area*meff*grav + 0.0 +Area*gam*GZZ12*0.5*(In+I0)/rho0 ))*fd.dx(degree=vpolyp)
    Z_exprforce   = ( vvmp3*( -Forcingterm ))*fd.dx(degree=vpolyp)
...
F_exprnl = phi_expr+h_exprm+q_expr+Z_exprm+W_expr+I_expr
F_exprnl0 = phi_expr+h_expr+q_expr+Z_expr+W_expr+I_expr
F_exprnls = F_exprnl0+h_exprforce+Z_exprforce
#  uu = fd.TrialFunction(mixed_Vmp) J_mid_full = fd.derivative(F_exprnlm, result_mixedmp)
...
solver_parameters4v = {
    ...
    'ksp_type': 'gmres',
    'pc_type': 'fieldsplit',
    'pc_fieldsplit_0_fields': '0',  # Continuous fields h, phi, q
    'pc_fieldsplit_1_fields': '1,2',    # R variables Z, W
    'pc_fieldsplit_0': {
        'type': 'schur',
        'schur_factorization_type': 'full',
        'schur_precondition': 'selfp',
...
solver_parameters24= {
    'snes_type': 'newtonls',
    "snes_linesearch_type": "bt",       # CRITICAL for contact nonlinearity
    "snes_linesearch_maxit": 50,
    "snes_linesearch_scale_l": True,
    "snes_monitor": None,
    'snes': {
        'rtol': 1e-9,
        'atol': 1e-10,
    ...
    'pc_type': 'fieldsplit',
    'pc_fieldsplit_0_fields': '0,1,2',  # Continuous fields h, phi, q
    'pc_fieldsplit_1_fields': '3,4,5',  # R variables Zn, Wn, In (3 of them)
    'pc_fieldsplit_0': {
        'type': 'schur',
        'schur_factorization_type': 'full',
        'schur_precondition': 'selfp',
    ...
    'pc_fieldsplit_1': {
        'mat_type': 'nest',              # CRITICAL: nest inside fieldsplit_1
        'ksp_type': 'preonly',
        'pc_type': 'lu',
    },
}
...
if nvpcase=="AVF":
    ...
    # Hydrostatic solvera: 
    wavebuoy_hydrotstaticnlshit = fd.NonlinearVariationalSolver(fd.NonlinearVariationalProblem(F_hyexprnlsh, result_mixedmp)) # , solver_parameters=solver_parameters_ss)
    wavebuoy_hydrotstaticnl = fd.NonlinearVariationalSolver(fd.NonlinearVariationalProblem(F_hyexprnl, result_mixedmp)) # , solver_parameters=solver_parameters20v) # , solver_parameters=solver_parameters20v)
    # Dynamic solver:
    # wavebuoy_nl = fd.NonlinearVariationalSolver(fd.NonlinearVariationalProblem(F_exprnl, result_mixedmp,J=J_mid_full)) # , solver_parameters=solver_parameters4v)
    wavebuoy_nl = fd.NonlinearVariationalSolver(fd.NonlinearVariationalProblem(F_exprnl, result_mixedmp)) # , solver_parameters=solver_parameters4v)
...
# cc_values = [10, 100.0, 500.0, 1000.0, 2000.0] # was up to 500 1000 (CG2, 14) 2000, 4000 , 4000.0, 5000.0, 7000.0] # , 7000.0, 10000.0]
# cc_values = [ bb**(-1)] cc_values = [0.01/bb, 0.015/bb, 0.02/bb]
cc_values = [0.001*bbb**(-1),0.01*bbb**(-1), 0.015*bbb**(-1), 0.02*bbb**(-1), 0.03*bbb**(-1), 0.04*bbb**(-1),0.05*bbb**(-1),0.06*bbb**(-1),0.07*bbb**(-1),0.1*bbb**(-1),0.15*bbb**(-1),0.2*bbb**(-1),\
             0.25*bbb**(-1),0.3*bbb**(-1),0.35*bbb**(-1),0.4*bbb**(-1),0.45*bbb**(-1),0.5*bbb**(-1),0.55*bbb**(-1),0.6*bbb**(-1),0.65*bbb**(-1),0.7*bbb**(-1),0.75*bbb**(-1),\
             0.8*bbb**(-1),0.85*bbb**(-1),0.9*bbb**(-1),0.95*bbb**(-1),0.975*bbb**(-1),bbb**(-1)]

#
cc_values = []
print("Starting shit continuation loop...")
for cc_val in cc_values:
    # Update the Firedrake Constant with the new cc value cc.assign(cc_val)
    cc.assign(cc_val)
    print(f"Solving shit for cc = {cc_val}...")
    solver_reason = wavebuoy_hydrotstaticnlshit.solve()
    if nvpcase=="AVF":
        phin, hn, qn, Zn, Wn, In = fd.split(result_mixedmp)
    else:
        Un, Zn, Wn, In = fd.split(result_mixedmp)
    ...
    print(f"Solver for cc={cc_val} converged reason: {solver_reason}; more shit Zn", Z00)

nshit=1
if nshit==0:
    snes = wavebuoy_hydrotstaticnl.snes
    lb = result_mixedmp.copy()
    ub = result_mixedmp.copy()
    with lb.dat.vec as v: # 4. Set unconstrained components to extreme values
        v.set(-1e10) # Everything else can go as low as it wants
    with ub.dat.vec as v:
        v.set(1e10)  # Everything else can go as high as it wants
    lb.sub(1).assign(0.01*H0) # 5. Lock Zn (sub-function 1) to be positive
    with lb.dat.vec_ro as v_lb, ub.dat.vec_ro as v_ub: # 6. Tell the PETSc SNES to use these vectors for bounds
        snes.setVariableBounds(v_lb, v_ub)
else:
    if nvpcase=="AVF":
        phin, hn, qn, Zn, Wn, In = fd.split(result_mixedmp)
    else:
...
# bb_values = [1000*bbb,100*bbb,10*bbb,bbb] # 1000*bb,100*bb,99*bb,98*bb,10*bb,bb]
bb_values = [bbb,bbb] # [100*bbb,10*bbb,bbb,0.1*bbb, 0.01*bbb, 0.001*bbb,10**(-4)*bbb,10**(-8)*bbb] # 1000*bb,100*bb,99*bb,98*bb,10*bb,bb]
...
for bb_val in bb_values:
    ...
    bb.assign(bb_val)
    dd.assign(bb_val)
    print(f"Solving for bb/cc = {cc_val}...",cc_val)
    ...
    print(f"Solver for cc={cc_val} converged reason: {solver_reason}; Zn", Z00)
...
# Initial plot update
# OB if animate_mode and nmpi==0:
# OB    update_plots(fig, axes, fig3, ax1_fig3, t, Z00, W00, h1vals, phi1vals, q1vals, yvals, yvLw, Lw, Ly, Z1, Keel, tanalp,save_for_animation=True)
# OBelse:
# OB    if nmpi==0:
# OB        update_plotstatic(fig, axes, fig3, ax1_fig3, t, Z00, W00, h1vals, phi1vals, q1vals, yvals, yvLw, Lw, Ly, Z1, Keel, tanalp)
...
# lam1.interpolate( nwave*( fd.conditional( fd.lt(Ch,-delta), aadbb*Ch, fd.conditional( fd.lt(Ch, delta), coeff_B*((Ch-delta)**4+LL*(Ch-delta)**3)+coeff_C*(Ch-delta)**3, 0.0) ) ) )
lam1.interpolate( nwave*lamb6(Ch) )
...
E0 = fd.assemble( ( 0.5*hn*fd.inner(fd.grad(phin)+ah01*hn*qn*fd.grad(hn)+aq01*(1/3)*hn**2*fd.grad(qn),fd.grad(phin)+ah01*hn*qn*fd.grad(hn)+aq01*(1/3)*hn**2*fd.grad(qn))+0.5*grav*fd.inner(hn-H0,hn-H0) \
                    +aq02*(1/6)*hn**3*qn**2+(betaa/90)*hn**5*fd.inner(fd.grad(qn),fd.grad(qn))+(nwave*ggrav/cc)*fd.exp(-cc*(Zn-hn-Keel-tanalp*(x[1]-Ly)))\
...
while t <= 1.0*(t_end + dt):
    ...
    if (t in t_plot): # Plotting # print('Plotting starts')
        ...
        E1 = fd.assemble( ( 0.5*hn*fd.inner(fd.grad(phin)+ah01*hn*qn*fd.grad(hn)+aq01*(1/3)*hn**2*fd.grad(qn),fd.grad(phin)+ah01*hn*qn*fd.grad(hn)+aq01*(1/3)*hn**2*fd.grad(qn))+0.5*grav*fd.inner(hn-H0,hn-H0) \
                            + aq02*(1/6)*hn**3*qn**2+(betaa/90)*hn**5*fd.inner(fd.grad(qn),fd.grad(qn)) \
                            + V_lamb5( (Zn-hn-Keel-tanalp*(x[1]-Ly)) ) \
                            + nwave*Area*meff*(0.5*Wn**2+grav*Zn) + Area*0.5*Li*In**2/rho0 )*fd.dx(degree=vpolyp) )
        # (nwave*ggrav/cc)*fd.exp(-cc*(Zn-hn-Keel-tanalp*(x[1]-Ly)))\
        ...
        #        plt.draw()
        #        plt.pause(0.00001) 
        # lam1.interpolate( -(nwave*ggrav)*fd.exp(-cc*(Zn-hn-Keel-tanalp*(x[1]-Ly))) )
        # fd.conditional( fd.ge(Ch,0.0), 0.0, -aa*(fd.sqrt(1.0+(Ch/bb)**2)-1.0) )
        # lam1.interpolate( nwave*( fd.conditional( fd.lt(Ch,-delta), aadbb*Ch, fd.conditional( fd.lt(Ch, delta), coeff_B*((Ch-delta)**4+LL*(Ch-delta)**3)+coeff_C*(Ch-delta)**3, 0.0) ) ) )
        lam1.interpolate( nwave*lamb6(Ch) )
        outfile_phin.write(phi1, time=t)
        outfile_psin.write(q1, time=t)
