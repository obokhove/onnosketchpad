...
vvmpc0, vvmpc1, vvmpc2, vvmpc3, vvmpc4, vvmpc5  = fd.split(vvmpc) # These represent "blocks".
Xh12, Uh12, Yh12, Vh12,lamb12, Ffunc = fd.split(result_mixedmpc)
...
while t <= 1.0*(t_end + dt): #
    ...
    if (t in t_plot): #
        ...
        ax2_right = ax2.twinx()
        ax2_right.plot(t, E1, '.b', label="H(t)")  # Plot E1 in blue
        ax2_right.set_ylabel(r"H(t)", color='b')
        ax1.plot(XXp,YYpu,'-b')
        ax1.plot(XXp,YYpm,'-b')
        ax4_right = ax4.twinx()  # Create a secondary y-axis
        ax4.plot(t, F0p, '.r', label="G(X,Y)")  # Original plot on ax4 (left y-axis)
        ax4_right.plot(t, lam0p, '.b', label=r"$\lambda$")  # Plot lamb12 on the right y-axis
        ax4.set_ylabel("G(X,Y)", color='r')
        ax4_right.set_ylabel(r"$\lambda$", color='b')
        ax4_right.tick_params(axis='y', labelcolor='b')  # Set color for readability
        ax4.tick_params(axis='y', labelcolor='r')  # Set color for readability  
        plt.tight_layout()
        plt.pause(0.1)
