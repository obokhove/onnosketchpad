...
if nwave==1:
    ...
    if nlatch==1:
        # Latching state variables
        latch_state   = 'none'        # current latch status ('none' or 'low')
        trigger_timer = 0.0           # accumulates time while latch condition is met
        Zf = Zbar
        Z_set         = None          # stores the Z value where latch engages
        control_term  = 0.0*Zn           # scalar term for weak form 
        # Latching control parameters
        t_start       = 2.0       # [s] only allow latch after this time
        Z_low_on      = -0.10     # trigger threshold to engage latch
        Z_low_off     = -0.08     # release threshold (hysteresis band) debounce_time = 0.05      # [s] condition must persist
        debounce_time = 0.5*Twm
        alpha_filter  = 0.2       # smoothing for Z measurement (0 = no filter, 1 = instant)
        Kp_latch      = 1e4       # proportional gain (used in Z_expr outside loop)
        Kd_latch      = 1e2       # derivative gain (used in Z_expr outside loop)
        # assume Zn is current/new, Z0 previous; Wn, W0 velocities etc.
        control_term_const = fd.Constant(control_term)  # initial Constant
        Z_expr = Z_expr + ( vvmp3 * Area * control_term_const ) * fd.dx(degree=vpolyp)
else:
...
while t <= 1.0*(t_end + dt):
    ...
    if nlatch == 1:
        # --- Step 1: update latch state ---
        Zf = alpha_filter*Z00[0] + (1.0 - alpha_filter)*Zf
        if t >= t_start:
            if latch_state != 'low' and Zf <= Z_low_on:       # candidate: engage low latch
                trigger_timer += dt
                if trigger_timer >= debounce_time:
                    latch_state = 'low'
                    Z_set = Zf             # lock Z at current low point
                    trigger_timer = 0.0
            elif latch_state == 'low' and Zf >= Z_low_off:    # candidate: release latch
                trigger_timer += dt
                if trigger_timer >= debounce_time:
                    latch_state = 'none'
                    Z_set = None
            ...
            Z_set = None
            trigger_timer = 0.0
        # --- Step 2: compute control term ---
        if latch_state == 'low' and Z_set is not None:
            Z_avg = 0.5*(Zn + Z0)
            W_avg = 0.5*(Wn + W0)
            Z_err = Z_avg - fd.Constant(Z_set)   # wrap scalar as Constant
            control_term = Kp_latch*Z_err + Kd_latch*W_avg
        else:
            control_term = 0.0*Zn                 # type-matched zero
        # --- Step 3: update Constant in weak form ---
        control_term_const.assign(control_term)
    else:
        # latching disabled
        latch_state = 'none'
        Z_set = None
        control_term_const.assign(0.0*Zn)
    # End latch
